...
use astarte_fdo_protocol::v101::randezvous_info::{
...
impl To1<Hello> {
    ...
    pub fn new(device_creds: DeviceCredential<'static>) -> Self {
        Self {
            device_creds,
            state: Hello {},
        }
    ...
    /// Tries to retrive the [`RvRedirect`].
    ...
    async fn run<C, S>(self, ctx: &mut Ctx<'_, C, S>) -> Result<To1<Ack>, Error>
    where
        C: Crypto,
    {
        let mut delay = None;
    ...
    async fn wait_for(&self, mut delay: Duration) -> Result<(), Error> {
        ...
        let add =
            i64::try_from(delay.as_secs().div_euclid(100).saturating_mul(25)).map_err(|err| {
...
impl To1<Ack> {
    async fn run<C, S>(self, ctx: &mut Ctx<'_, C, S>) -> Result<To1<Prove>, Error>
    where
        C: Crypto,
    {

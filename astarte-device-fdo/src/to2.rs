...
use astarte_fdo_protocol::v101::sign_info::{EASigInfo, EBSigInfo, SigInfo};
...
impl To2<Hello> {
        ...
        C: Crypto,
    {
        ...
        Err(Error::new(
            ErrorKind::Io,
            "to connect with the ownerhsip server",
        ))
    }
...
impl To2<Prove> {
        ...
        C: Crypto,
    {
        ...
        C::verify_cose_singature(self.state.rv.to1d(), hdr.pubkey())?;

        info!("TO2.ProveOvHdr RvRedirect verified");

        C::verify_cose_singature(self.state.hdr.sign(), hdr.pubkey())?;
...
impl To2<VerifyChain> {
        ...
        C: Crypto,
    {
        ...
        Ok(To2 {
            ...
            state: ProveDv {
                ...
                nonce_to2_prove_dv: self.state.hdr.take_nonce(),
    ...
    {
        // 1. Verify signature TO2.OVNextEntry.OVEntry using variable PubKey
        C::verify_cose_singature(entry.ov_entry().sing(), &variables.pub_key)
            .inspect_err(|_| error!("coudln't validate ov entry signature"))?;
        info!("cose signature verified");

        // 2. Verify variable HashHdrInfo matches TO2.OVEntry.OVEHashHdrInfo
        let (entry, payload) = entry.tak_ov_entry().payload()?;
    ...
    C: Crypto,
{
        ...
        C: Crypto,
    {
        ...
        C::verify_cose_singature(setup_dv.sign(), payload.ow_pubkey())?;
